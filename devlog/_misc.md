...
## 2026-06-26: 代码审查跟进—线程安全锁 + 标签页统一
- **文件:**
  - `得物对账单_sqlserver版.py`
  - `得物对账单_历史数据版.py`
- **原因:** 审查报告 M1/M2/L1 跟进修复
- **修复:**
...
## 2026-06-27: _run_task 竞态修复—auto_run 检查移入锁内
- **文件:** `得物对账单_sqlserver版.py`
- **原因:** 审查报告指出 `auto_run.running` 检查在锁外，存在极小竞态窗口（自动运行刚停止但用户仍被误拒，或自动运行刚启动但手动任务已通过检查）
- **修复:** 将 `auto_run.running` 检查与 `_running_tasks` 检查合并移入 `_task_lock`，保证检查与状态更新的原子性
- **影响范围:** 仅 `_run_task` 方法逻辑重排，无功能变化
