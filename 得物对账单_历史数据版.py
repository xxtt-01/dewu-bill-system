...
class DBConnection:
    ...
    def __enter__(self):
        ...
        try:
            self.conn = pyodbc.connect(conn_str, fast_executemany=True)
    ...
    while True:
        ...
        try:
            ...
            if result.get("code") != 200:
                err_msg = f"API返回错误码: code={result.get('code')}, msg={result.get('message', '')}"
                logging.warning(f"[{credential.cred_id}] {err_msg}")
                logging.warning(f"完整响应: {str(result)[:500]}")
            ...
            logging.error(f"API响应异常 [{credential.cred_id}]: {str(e)}")
            raise

        data = result.get("data", {})
        if not isinstance(data, dict):
            logging.warning(f"API返回data字段非字典 [{credential.cred_id}]: type={type(data).__name__}，跳过该页")
            break
        ...
        for bill_no in self.bill_nos:
            try:
                ...
                if isinstance(e, RetryError) and hasattr(e, 'last_attempt'):
                    ...
                    err_msg = f"重试耗尽: {type(original).__name__}: {original}"
                self.results[bill_no] = {'success': False, 'error': err_msg}
                logging.error(f"处理账单失败 [{self.credential.cred_id}]: {err_msg}", exc_info=True)
    ...
    with ThreadPoolExecutor(max_workers=DOWNLOAD_WORKERS) as executor:
        ...
        for f in as_completed(futures):
            ...
            try:
                ...
                logging.error(f"下载失败 [{bill_no}]: {str(e)}", exc_info=True)
                completed_files += 1
...
def run_processing(root, update_log, start_date=None, end_date=None):
    ...
    try:
        ...
        def _process_one_shop(credential):
            """处理单个店铺（串行处理，得物API限流）"""
            shop_log = lambda msg: update_log(msg, tab=credential.cred_id)
            logging.info(f"=== 开始处理店铺: {credential.cred_id} ===")
            shop_log(f"开始处理店铺: {credential.cred_id}")
            inserted_records = []
            download_results = {}
            skipped_records = {}
            ...
            try:
                ...
                if api_response.get("code") != 200:
                    err_detail = f"code={api_response.get('code')}, msg={str(api_response.get('message', ''))}"
                    logging.error(f"API业务错误 [{credential.cred_id}]: {err_detail}")
                    raise ValueError(f"API错误(code={api_response.get('code')})")
                ...
                if not processed_data:
                    ...
                    shop_log(f"没有可处理的有效数据 [{credential.cred_id}]")
                else:
                    with DBConnection() as cursor:
                ...
                if not bill_nos:
                    ...
                    shop_log(f"没有需要下载的账单 [{credential.cred_id}]")
                ...
                if bill_nos:
                    ...
                    shop_log(f"需下载 {len(bill_nos)} 个账单（含文件缺失重试）")

                processor = BillProcessor(bill_nos, credential, progress_callback=lambda x, y: shop_log(f"进度: {x}/{y}"))
                processor.process_all()
                download_results = processor.results
                shop_log(f"[{credential.cred_id}] 等待 {DOWNLOAD_WAIT_SECONDS} 秒后开始下载...")
                time.sleep(DOWNLOAD_WAIT_SECONDS)
                download_files(download_results, credential.cred_id, progress_callback=lambda x, y: shop_log(f"下载进度: {x}/{y}"))
                return {"execution_time": execution_time, "shop_name": credential.cred_id, "status": "成功",
                ...
                if isinstance(e, RetryError) and hasattr(e, 'last_attempt'):
                    original = e.last_attempt.exception()
                    err_msg = f"重试耗尽: {type(original).__name__}: {original}"
                logging.error(f"处理失败 [{credential.cred_id}]: {err_msg}", exc_info=True)
                shop_log(f"处理失败 [{credential.cred_id}]: {err_msg}")
...
def import_bills(root, update_log, start_date=None, end_date=None):
    ...
    try:
                ...
                for file in os.listdir(shop_path):
                    if file.endswith('.xlsx') and not file.endswith('_tiqu.xlsx') and not file.startswith('~$'):
                        ...
                        if not need_process and check_file_changed(src_path, tracking_file):
                            ...
                            update_log(f"检测到文件更新，重新生成 tiqu: {file}", tab=shop_folder)
                            need_process = True
                        if need_process and os.path.exists(src_path):
                            all_files.append((src_path, dest_dir, dest_path, shop_folder))
        ...
        for i, (src_path, dest_dir, dest_path, shop_name) in enumerate(all_files):
            try:
                os.makedirs(dest_dir, exist_ok=True)
                logging.info(f"开始处理文件: {src_path}")
                update_log(f"正在处理: {src_path}", tab=shop_name)
                ...
                update_log(f"文件处理完成: {src_path} -> {dest_path}", tab=shop_name)
                update_log(f"进度: {i + 1}/{total_files}", tab=shop_name)

            except Exception as e:
                error_msg = f"处理失败 {src_path}: {str(e)}"
                logging.error(error_msg, exc_info=True)
                update_log(f"失败: {src_path} - {str(e)}", tab=shop_name)
                continue

        # 更新文件追踪
        for src_path, _, _, _ in all_files:
...
def test_db_connection_gui(update_log, msg_signal=None):
    """GUI 测试数据库连接"""
    try:
        ...
        if test_db_connection():
            update_log("数据库连接成功！")
            if msg_signal:
                msg_signal.emit("info", "成功", "数据库连接测试成功！")
        else:
            update_log("数据库连接失败！")
            if msg_signal:
                msg_signal.emit("error", "失败", "数据库连接测试失败！")
    except Exception as e:
...
class MainWindow(QMainWindow):
    ...
    def _setup_ui(self):
        ...
        self.tab_widget.setStyleSheet(f"""
            ...
            QTabBar::tab:selected {{
                background-color: {self.C_CARD};
                color: {self.C_ACCENT};
                border-bottom: 2px solid {self.C_ACCENT};
            }}
        ...
        buttons = [
            ...
            ("测试连接", lambda: self._run_task("测试连接",
                lambda: test_db_connection_gui(self._update_log, self.msgbox_signal))),
        ]
    ...
    def _ensure_tab(self, tab_name: str):
        """按名称创建或获取日志 tab"""
        if tab_name in self.log_tabs:
            return
        text_edit = self._create_log_text_edit()
        self.tab_widget.addTab(text_edit, tab_name)
        self.log_tabs[tab_name] = text_edit

    def _append_log(self, tab_name, message):
        """主线程中路由日志到对应 tab（由信号触发）"""
        if tab_name not in self.log_tabs:
            self._ensure_tab(tab_name)
        self.log_tabs[tab_name].appendPlainText(message)
    ...
    def _show_msgbox(self, msg_type, title, message):
        """主线程中显示消息弹窗（由信号触发）"""
        if msg_type == "info":
    ...
    def _update_log(self, message, tab="总览"):
        """从工作线程调用的日志更新（线程安全）"""
        ...
        if reply == QMessageBox.StandardButton.Yes:
            ...
            with self._task_lock:
                if self._running_tasks:
                    logging.warning(f"窗口关闭，正在运行的任务可能被中断: {self._running_tasks}")
            event.accept()
        else:
            event.ignore()
